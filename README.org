#+TITLE: Emacs 配置（个人速查）

个人 Emacs 配置：轻量、稳定、Vim-like，重点服务写作与轻量代码阅读。

* 我日常怎么用

- 命令入口：=SPC SPC=（输入命令名，例如 =load-my-config=）
- 找文件：=SPC f f=
- 全文检索：=SPC f g=
- 切项目：=SPC p p=
- Capture 前缀：=SPC a o c=
- Capture 子键：=b= brainstorm / =i= idea / =j= journal / =m= meeting / =c= dispatch
- 搜索候选动作：=C-.= 或 =SPC a .=（动作） / =C-c C-o=（导出为可批处理列表）
- 分屏窗口移动：=SPC w h/j/k/l=
- 字号调节：=SPC z ==（放大）/=SPC z -=（缩小）/=SPC z 0=（重置）

* 笔记工作流

- 默认目录：=~/Documents/orgfiles/=
- 本机覆盖：环境变量 =ORGFILES_ROOT= 或 =~/.emacs.d/local.el=
- Idea：捕获到 =~/Documents/orgfiles/ideas.org=（可被本机覆盖）
- Brainstorm：每次新建到 =~/Documents/orgfiles/YYYY-MM-DD-HHMM-<标题>.org=（可被本机覆盖）
- Journal：捕获到 =~/Documents/orgfiles/YYYY-MM-DD.org=（可被本机覆盖）
- Meeting：每次新建到 =~/Documents/orgfiles/YYYY-MM-DD-HHMM-<会议名>.org=（可被本机覆盖）

#+begin_src emacs-lisp
;; ~/.emacs.d/local.el
(setq my/orgfiles-root "~/Dropbox/orgfiles/")
#+end_src

* 新机器初始化

1. Clone 这个仓库到 =~/.emacs.d=。
2. 如果你接受默认目录，直接创建 =~/Documents/orgfiles/=。
3. 如果你要用自己的同步目录，复制 [[./local.example.el][local.example.el]] 为 =~/.emacs.d/local.el= 并改成你自己的路径。
4. 或者不用 =local.el=，直接在 shell 里设置环境变量 =ORGFILES_ROOT=。

#+begin_src sh
# 方案 A：用默认目录
mkdir -p ~/Documents/orgfiles

# 方案 B：用环境变量覆盖
export ORGFILES_ROOT="$HOME/Dropbox/orgfiles"
#+end_src

说明：
- =local.el= 已在 Git 中忽略，适合放本机私有路径。
- =ORGFILES_ROOT= 优先级更高，适合不同机器临时切换目录。

* 配置入口

- 主配置：[[./config.org][config.org]]
- 启动入口：[[./init.el][init.el]]
- 回归测试：[[./tests/config-tests.el][tests/config-tests.el]]
- 设计规范：[[./doc/PROJECT_REQUIREMENTS.md][doc/PROJECT_REQUIREMENTS.md]]
- 完整快捷键：[[./doc/KEYMAPS.org][doc/KEYMAPS.org]]

* 常用命令

#+begin_src sh
# 跑回归测试
emacs --batch -l tests/config-tests.el -f ert-run-tests-batch-and-exit

# 验证配置可加载
emacs --batch -l init.el
#+end_src
